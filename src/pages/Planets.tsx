...
import { PlanetsRowColLayout, PlanetsTableLayout } from 'components/planets';
  ...
  useEffect(() => {
    ...
    const planetsData = async () => {
      try {
        ...
        const res = await getPlanets(paramsPage);
    ...
    >
      ...
      {loading === false && showError === false ? (
        <Container maxWidth="xl">
          ...
          {layout !== 'table' ? (
            <PlanetsRowColLayout layout={layout} data={planets} />
          ) : (
            <PlanetsTableLayout data={planets} />
          )}
          {prevPage || nextPage ? (
            <Pagination
